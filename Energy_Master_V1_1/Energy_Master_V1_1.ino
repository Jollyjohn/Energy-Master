...
void setup() {
  ...
  // sets the input pin as input with a 10k resistor to VCC
...
void loop() {
  ...
  // Stop the Ethernet routines from being interupted
  noInterrupts();
...
//  lcd.println(strlen(COSM_data));
  ...
  if (client.connect("api.cosm.com", COSM_port)) {
      ...
      client.println("PUT /v2/feeds/42622.csv HTTP/1.1");   // Send the HTTP PUT request. This is a Pachube V2 format request. The .csv tells COSM what format we are using 
